...
import itemTextSnapshot from "./chrome.itemText.snapshot.json";
import { macOSRecord } from "../../../../lib";
import spokenPhraseSnapshot from "./chrome.spokenPhrase.snapshot.json";
import { voTest as test } from "../../voiceover-test";

test.describe("Chrome Playwright VoiceOver", () => {
  test("I can navigate the Guidepup Github page", async ({
    page,
    voiceOver,
  }) => {
    const stopRecording = macOSRecord(
      `./recordings/playwright-voiceover-chrome-${+new Date()}.mov`
    );
